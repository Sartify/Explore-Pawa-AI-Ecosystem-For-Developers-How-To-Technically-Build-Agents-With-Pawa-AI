...
from app.services.tools import search_buses, book_bus, cancel_booking

load_dotenv()

PAWA_API_KEY = os.getenv("PAWA_API_KEY")
PAWA_API_URL = os.getenv("PAWA_API_URL", "https://api.pawa-ai.com/v1/chat/request")
MODEL_NAME = os.getenv("MODEL_NAME", "pawa-v1-blaze-20250318")


class PawaClient:
    """Handles communication with Pawa AI chat model (async)"""
    ...
    async def send_message(self, system_prompt: str, conversation: list, user_input: str) -> dict:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(self.url, json=payload, headers=headers, timeout=30) as resp:
                if resp.status != 200:
                    ...
                    raise Exception(f"Pawa API Error {resp.status}: {text}")
        ...
        if tool_calls:
            for call in tool_calls:
                ...
                if func.get("name") == "cancel_booking":
                    return {"reply": cancel_booking(args)}
